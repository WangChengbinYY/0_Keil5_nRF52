...
TimerHandle_t     xTimerHandle_CollectData        = NULL;         /*10ms触发的采集任务    句柄 */
...
static void vTask_SDCard_Save(void *pvParameters)
{
        ...
        while(1)
    {
                    NRF_LOG_INFO("vTask_SDCard_Save Begin");
                    NRF_LOG_FLUSH();
        
        vTaskSuspend(NULL);
                            NRF_LOG_INFO("vTask_SDCard_Save Resume");
                    NRF_LOG_FLUSH();
...
////                    portMAX_DELAY);       /* 最大允许延迟时间 portMAX_DELAY 表示永远等待*/
////TEST        
//NRF_LOG_INFO("vTask_SDCard_Save  Fore!!");
//NRF_LOG_FLUSH();         
//        
//        
...
//        
////TEST        
//NRF_LOG_INFO("vTask_SDCard_Save!!");
//NRF_LOG_FLUSH();  
...
{
                NRF_LOG_INFO("      vTimer_CollectData Start");
                NRF_LOG_FLUSH();     
                
    //nrf_delay_ms(100);
    
    vTaskResume(xTaskHandle_SDCard_Save);  
