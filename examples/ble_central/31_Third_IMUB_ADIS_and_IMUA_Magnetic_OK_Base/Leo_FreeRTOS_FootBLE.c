...
extern uint8_t      G_FOOTPressure_Data[25];            //足部压力传感器数据
extern uint8_t      G_FOOTPressure_Counter;             //足部压力传感器数据的计数器
extern uint32_t     G_GPSWeekSecond;                   //GPS周内秒数据
extern uint16_t     G_MicroSecond; 

extern TaskHandle_t    xTaskHandle_FootPres_GetData; 
/*
...
#define SCAN_INTERVAL           0x01A0                                  /**< Determines scan interval in units of 0.625 millisecond. */
#define SCAN_WINDOW             0x0150                                  /**< Determines scan window in units of 0.625 millisecond. */
#define SCAN_DURATION           0x0000                                  /**< Timout when scanning. 0x0000 disables timeout. */

#define MIN_CONNECTION_INTERVAL MSEC_TO_UNITS(800, UNIT_1_25_MS)         /**< Determines minimum connection interval in millisecond. */
#define MAX_CONNECTION_INTERVAL MSEC_TO_UNITS(1600, UNIT_1_25_MS)         /**< Determines maximum connection interval in millisecond. */
...
void ble_nus_chars_received_uart_print(uint8_t * p_data, uint16_t data_len)
{
    ...
    if(G_FOOTPressure_Counter == 255)
    {
        G_FOOTPressure_Counter = 0;
    }else
    {
        G_FOOTPressure_Counter++;
    }
    memcpy(G_FOOTPressure_Data+2,&G_GPSWeekSecond,4);
    memcpy(G_FOOTPressure_Data+6,&G_MicroSecond,2);      
    memcpy(G_FOOTPressure_Data+8,&G_FOOTPressure_Counter,1); 
    memcpy(G_FOOTPressure_Data+9,&p_data,data_len);     
    
    xTaskNotifyFromISR(xTaskHandle_FootPres_GetData,      /* 目标任务 */
                       0,                               /* 发送数据 */
    ...
    if((G_FOOTPressure_Counter % 100) == 0)
    {
        NRF_LOG_INFO("Podoon Data is OK!___1s,the length is %d ",data_len);
    }
...
void ble_nus_c_evt_handler(ble_nus_c_t * p_ble_nus_c, ble_nus_c_evt_t const * p_ble_nus_evt)
{
    ...
    switch (p_ble_nus_evt->evt_type)
    {
        case BLE_NUS_C_EVT_DISCOVERY_COMPLETE:
            ...
            /* 连接以后，给对方发送 “传输数据的指令” */
            char begin = 69; 
			ble_nus_c_string_send(p_ble_nus_c, (uint8_t *)(&begin), 1);
            break;
