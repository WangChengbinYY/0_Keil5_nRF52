...
{
//    nrfx_gpiote_out_clear(configGPIO_SPI_UWB_nCS);       
//    nrf_delay_us(1);  
    ...
    //2.开发发送 发送之后即可转入 待接收状态 延迟由dwt_setrxaftertxdelay()设定，此处不设定，不延迟等待
...
//4.收到反馈     
    ...
    {
                NRF_LOG_INFO("UWB INIT RX is OK");
                NRF_LOG_FLUSH();         
...
//                dwt_rxreset();
        ...
        {
            NRF_LOG_INFO("UWB INIT Message is Wrong!");
            NRF_LOG_FLUSH(); 
...
uint8_t ucSS_RESP_Initial(void)
{
    ...
    dwt_setinterrupt(DWT_INT_RFCG | configUWB_RESP_SYSMASK_ALL_RX_ERR | DWT_INT_TFRS,1);
...
uint8_t ucSS_RESP_Handler(void)
{
    ...
    if (status_reg & SYS_STATUS_RXFCG)
    {
        ...
        if (memcmp(RSP_rx_buffer, RSP_rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
        {
            ...
            {
                //发送失败的话
                NRF_LOG_INFO("UWB Resp TX Fault!");
                NRF_LOG_FLUSH(); 
                dwt_rxreset(); 
                dwt_rxenable(DWT_START_RX_IMMEDIATE);
                return 1;                
