...
/*
    SPI通信事件处理函数
*/
static void Leo_IMU_SPI_Event_Handler(nrf_drv_spi_evt_t const * p_event,void * p_context){
...
//    G_MPU9255_SPI_xfer_Done = 0;
	error_code = nrf_drv_spi_transfer(&SPI_CollectData, w2_data,2, r2_data, read_number);	
...
//	*(Data+4) = buf[6];
//	*(Data+5) = buf[5];	    
    error_code =Leo_MPU9255_SPI_ReadBytes(MPU9255_EXT_SENS_DATA_00,Data,8);
	return error_code;	
...
/**
 初始化 IMUA(MPU9250)  同时使用IMUA(MPU9250)和IMUB(ADIS)
*/
uint8_t ucIMU_INIT_IMUA_MPU_and_IMUB_ADIS(void)
{
    //1.关闭IMUB
    uint8_t error_code = 0;
    ...
    nrfx_gpiote_out_uninit(configGPIO_SPI_IMUA_nCS); 
    error_code |= nrfx_gpiote_out_init(configGPIO_SPI_IMUA_nCS,&tconfigGPIO_OUT);
    nrfx_gpiote_out_set(configGPIO_SPI_IMUA_nCS);   
    nrf_delay_ms(2);
    
    //5.初始化数据读取使用的 无片选的 SPI
...
/**
 初始化 IMUA(MPU9250) 和IMUB(MPU9250) 
*/
uint8_t ucIMU_INIT_IMUAB_MPU(void)
{
    uint8_t error_code = 0;
    
//1. 先初始化IMUA    
    //1.1 直接将IMU_B(MPU925)的片选置高 
    nrfx_gpiote_out_uninit(configGPIO_SPI_IMUB_nCS);  
    nrfx_gpiote_out_config_t tconfigGPIO_OUT =  NRFX_GPIOTE_CONFIG_OUT_SIMPLE(true);
    error_code |= nrfx_gpiote_out_init(configGPIO_SPI_IMUB_nCS,&tconfigGPIO_OUT);
    nrfx_gpiote_out_set(configGPIO_SPI_IMUB_nCS);  //输出1  
...
/**
 初始化 IMUB(MPU9250)only First板子,不使用IMUA(MPU9250)
*/
uint8_t ucIMU_INIT_IMUBOnly_MPU(void)
{
    //1.关闭IMUA
    uint8_t error_code = 0;
    
    //直接将IMU_A(MPU92)的片选置高 
    nrfx_gpiote_out_uninit(configGPIO_SPI_IMUA_nCS);  
    nrfx_gpiote_out_config_t tconfigGPIO_OUT =  NRFX_GPIOTE_CONFIG_OUT_SIMPLE(true);
    error_code |= nrfx_gpiote_out_init(configGPIO_SPI_IMUA_nCS,&tconfigGPIO_OUT);
    nrfx_gpiote_out_set(configGPIO_SPI_IMUA_nCS);  //输出1  
    ...
    //2.初始化 IMUB(MPU92)的SPI设置
    nrf_drv_spi_config_t SPI_config = NRF_DRV_SPI_DEFAULT_CONFIG;
	SPI_config.sck_pin 			= configGPIO_SPI_CollectData_SCK;
	SPI_config.mosi_pin 		= configGPIO_SPI_CollectData_MOSI;
	SPI_config.miso_pin 		= configGPIO_SPI_CollectData_MISO;   
    SPI_config.ss_pin			= configGPIO_SPI_IMUB_nCS;               //不设定片选管脚
	SPI_config.irq_priority	    = SPI_DEFAULT_CONFIG_IRQ_PRIORITY;		//系统SPI中断权限默认设定为 7 
	SPI_config.orc				= 0xFF;
	SPI_config.frequency		= NRF_DRV_SPI_FREQ_1M;		        
	SPI_config.mode             = NRF_DRV_SPI_MODE_3;                     
