...
uint8_t     G_IMUA_MPU92[4];

//全局变量_数据存储_IMUA——为ADIS采集MPU9250的磁强计数据
uint8_t     G_IMUA_MPU92_Magnetic[10];
...
//全局变量_数据存储_IMUA——为ADIS采集MPU9250的磁强计数据
    memset(G_IMUA_MPU92_Magnetic,0,10);
...
static void vTask_CollectData_IMUA(void *pvParameters)
{
    ...
    while(1)
    {
        xTaskNotifyWait(0x00000000,0xFFFFFFFF,NULL,portMAX_DELAY);               
                //等待 共用SPI互斥量的释放
        if(xSemaphoreTake( xMutex_SDCard_CirBuffer, ( TickType_t ) 2 ) == pdTRUE)
        { 
            //采集IMUA MPU92数据
...
//            nrfx_saadc_sample_convert(3,tSAResult+3);  
            ...
            if(error_IMUA == 0)
            {
                ...
                //再存入 IMUA数据包 34个字节                
                if((34+G_SDCard_CB_Counter) <= configSDCard_BufferSize)   //判断缓存是否溢出
                {
                    //存入 头标识和时间
                    ucCircleBuffer_SaveData(G_IMUA_MPU92,sizeof(G_IMUA_MPU92));
                    //存入 IMUA数据                    
                    ucCircleBuffer_SaveData(tIMUA_MPU92_Data,22);                      
//                    //存入 压力数据                    
...
/*------------------------------------------------------------
 *  以IMUB(U5)为主循环 采集ADIS,额外存储MPU9250的磁强计数据
 *------------------------------------------------------------*/
/*
static void vTask_CollectData_IMUA(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        { 
            ...
            nrfx_gpiote_out_clear(configGPIO_SPI_IMUA_nCS); 
            nrf_delay_us(1);
            //error_IMUA = Leo_MPU9255_Read_Magnetic(G_IMUA_MPU92_Magnetic+4);	
            error_IMUA = Leo_MPU9255_Read_ALLData(tIMUA_MPU92_Data);
            nrfx_gpiote_out_set(configGPIO_SPI_IMUA_nCS); 
            memcpy(G_IMUA_MPU92_Magnetic+4,tIMUA_MPU92_Data+15,6);
            ...
            {
                //数据采集错误
                NRF_LOG_INFO("  IMU_A_Magnetic CellectData is Wrong!");
                NRF_LOG_FLUSH();                 
            }
...
static void vTask_CollectData_IMUB(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        {
            NRF_LOG_INFO("  xMutex_SDCDBuffer IMUB TimeOUT");
            NRF_LOG_FLUSH();
        }  
    }        
}
...
//    //（4） 初始化 IMUB(ADIS)  关闭IMUA 打开IMUB 
//    erro_code |= ucIMU_INIT_MPU_ADIS();   
//    NRF_LOG_INFO(("||Initialize||-->IMUB_ADIS------->error  0x%x"),erro_code);    
    
//    //（4） 初始化 IMUA(MPU9250)  打开IMUA 关闭IMUB 
//    erro_code |= ucIMU_INIT_IMUA_MPU_Only();   
//    NRF_LOG_INFO(("||Initialize||-->IMUA_MPU------->error  0x%x"),erro_code);  
    
    //（4） 初始化 IMUA(MPU9250) + IMUB(ADIS) 
    erro_code |= ucIMU_INIT_IMUA_and_IMUB_ADIS();   
    NRF_LOG_INFO(("||Initialize||-->IMUA+IMUB------->error  0x%x"),erro_code);      
