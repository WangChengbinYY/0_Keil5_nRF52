...
/**
 初始化 IMUA(MPU9250)  打开IMUA 关闭IMUB 
    不用ADIS，只用MPU9250    IMU_A 对应MPU9250  IMU_B 对应ADIS
*/
uint8_t ucIMU_INIT_IMUA_MPU_Only(void);


/**
 初始化 IMUA(MPU9250) 和 IMUB(ADIS) 
    IMU_A 对应MPU9250  IMU_B 对应ADIS
*/
uint8_t ucIMU_INIT_IMUA_and_IMUB_ADIS(void);
