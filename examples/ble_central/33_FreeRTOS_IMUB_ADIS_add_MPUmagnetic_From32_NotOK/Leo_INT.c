...
uint8_t mMPU92_Magnetic_Count = 0;
uint8_t mIMB_Count = 0;
/*
*********************************************************************************************************
*                                       SDCard 存储暂停 外部中断
*********************************************************************************************************
...
/*-----------------------------------------------------------------------*/
/* IMU_A(U4) 中断 响应事件  以MPU92为主的正常采集                                               */
/*----------------------------------------------------------------------*/
/*
...
*/
/*-----------------------------------------------------------------------*/
/* IMU_A(U4) 中断 响应事件  以ADIS为主，额外采集MPU9250的磁强计          */
/*         100Hz,两个循环采集一次
/*----------------------------------------------------------------------*/
static void vINTHandler_IMUA(nrfx_gpiote_pin_t pin, nrf_gpiote_polarity_t actio)
{
    ...
    if(G_SDCard_FileIsOpen == 1)
    {   
        mMPU92_Magnetic_Count++;
        if((mMPU92_Magnetic_Count%5)==0)
        {
            //记录IMUA 磁强计采集时间
            memcpy(G_IMUA_MPU92+2,&G_MicroSecond,sizeof(G_MicroSecond));            
            //通知任务进行数据采集
